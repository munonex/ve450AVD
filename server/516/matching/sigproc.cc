...
// AAC file header 
// This header should be 7 bytes
typedef struct aacHeader_t {
	unsigned int syncword: 12; // 0xFFF
	unsigned int ID: 1;	// ID = 0: MPEG-4; ID=1: MPEG-2
	unsigned int layer: 2;	// 00
	unsigned int protection_absent: 1; // 1

	unsigned int profile: 2; // 5->(01)01: 2 for SSR
	unsigned int sampling_freq_index: 4; // 5->01(01): 0x4 for 44100Hz
	unsigned int private_bit: 1; // 0
	unsigned int channel_config: 3; // 010 for front-left + front-right
	unsigned int original_copy: 1; // 0
	unsigned int home: 1; 	// 0

	/* adts_variable_header */
	unsigned int copyright_id: 2; // 00
	unsigned int frame_size: 13; // aac_frame
	unsigned int buffer_fullness: 11; // 0x7ff	
	unsigned int num_raw_data: 2; // 00
} aacHeader;
...
typedef struct WavHeader_t {
	...
	// char sth[32];	// 32-byte sth
	// unsigned short int sth2; // 16-bit sth

	unsigned short int subchunk2id; // "data"
	unsigned short int subchunk2id2;	
	unsigned short int subchunk2size1; // = nsamples * nchannels * bitspersample / 8
	unsigned short int subchunk2size2;
...
float * wavread(char * fn, unsigned int * nsamples, unsigned int * freq) {
	...
	cout << "WavHeader.subchunk2size1: " << hex << header.subchunk2size1 <<'\n';
	cout << "WavHeader.subchunk2size2: " << hex << header.subchunk2size2 <<'\n';
	unsigned int subchunk2size_new;
	subchunk2size_new = (header.subchunk2size2 <<16) + header.subchunk2size1;
	cout << "WavHeader.subchunk2size: " << hex << subchunk2size_new <<'\n';
	...
	if (header.subchunk2id != 0x6164 || header.subchunk2id2 != 0x6174) {
	...
	subchunk2size = (header.subchunk2size2 << 16) + header.subchunk2size1;

	*nsamples = subchunk2size / (header.nchannels * header.bitspersample / 8);

	*freq = header.samplerate;

	printf("  Bytes: %d   Samples: %d   Time: %0.2fs\n  Channels: %d   Sample rate: %d   BitsPerSample: %d\n",
	       subchunk2size, *nsamples, (float) *nsamples / header.samplerate, header.nchannels, header.samplerate,
